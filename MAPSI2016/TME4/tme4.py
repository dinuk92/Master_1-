# -*- coding: utf-8 -*-
"""
Created on Tue Oct 11 10:47:06 2016
...
def normale_bidim(x,z,(mux,muz,sigmax,sigmaz,rho)):    
    expr1 = 1.0/(2 * m.pi * sigmax * sigmaz * m.sqrt(1 - rho**2))
    expr2 = -1.0 / (2 * (1 - rho**2))
    expr3 = ((x - mux) / sigmax)**2
    expr4 = (-2) * rho * ((x - mux) * (z - muz)) / (sigmax * sigmaz)
    expr5 = ((z - muz) / sigmaz)**2
    return (expr1 * (m.exp(expr2 * (expr3 + expr4 + expr5))))
...
def Q_i(data,current_params,current_weights):
    pi0 = current_weights[0]       
    pi1 = current_weights[1]
    tab = np.zeros((len(data),2))
    alpha0 = np.array([pi0 * normale_bidim(data[i][0],data[i][1],current_params[0]) for i in range(len(data))])  
    alpha1 = np.array([pi1 * normale_bidim(data[i][0],data[i][1],current_params[1]) for i in range(len(data))])  
    tab[:,0]=(alpha0)/(alpha0 + alpha1)  
    tab[:,1]=(alpha1)/(alpha0 + alpha1)
    return tab
