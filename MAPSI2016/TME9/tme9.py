# -*- coding: utf-8 -*-
"""
Created on Tue Nov 29 10:58:07 2016
...
def MetropolisHastings(mess,mu,A,tau,N):
    ...
    for i in range(N):
        taut = swapF(tau)
        messdec = decrypt(mess,taut)
        
        Lt = logLikelihood(mess,mu,A,tau.keys())
        Ltaut = logLikelihood(messdec,mu,A,taut.keys())    
        rapport =  (Lt / Ltaut)
        print rapport 
        if rapport < 0 :
...
print MetropolisHastings(secret2, mu, A, tau_init, 10000 )
"""