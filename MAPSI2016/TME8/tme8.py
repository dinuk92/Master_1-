# -*- coding: utf-8 -*-
"""
Created on Tue Nov 22 10:51:24 2016
...
b = 1./np.sum((len(Xgenes[:][:])-6)/3) 

print b

binter = np.array([[float(len(np.where(Genome==n)[0]))/len(Genome)for n in range(4)]])
...
l = len(gene)
codon_pos1 = Xgenes[0][::3]
codon_pos2 = Xgenes[0][1::3]
codon_pos3 = Xgenes[0][2::3]
#print codon_pos1




codon_pos = np.array([[g[i::3] for g in Xgenes] for i in range(3)])


#codon_pos = np.array([g[1::3] for g in Xgenes])
print "ttttttttttt"
print codon_pos
...
#print len(np.where(np.array(codon_pos1)==0)[0]) 

nb_codon1A = np.array([[float(len(np.where(codon_pos1==n)[0]))/len(codon_pos1)for n in range(4)]])
