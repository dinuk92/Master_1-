...
#from sklearn.ensemble.gradient_boosting import np_bool
...
dt.fit(datax[:100] ,datay[:100])
dt.predict(datax[6:,:])
print dt.score(datax ,datay)
dt.to_pdf("/tmp/test_tree3.pdf",fields)
# dessine l'arbre  dans un  fichier pdf



def splite ( datax , datay  , nbtrain   ):
    r = np.arange(len(datax))
    r =np.random.permutation(r)
    r = r.tolist()
    #print r
    trainx = datax[r[:nbtrain]]
    testx = datax[r[nbtrain:]]
    trainy = datay[r[:nbtrain]]
    testy = datay[r[nbtrain:]]
    return testx, testy , trainx , trainy
tesx,tesy,trax,tray  = splite (datax,datay,100)
print trax
print datax[1]
...
def dept_test(datax , datay , deptmax):
    res = np.zeros(deptmax)
    for i in range(1,deptmax+1):
        tesx, tesy, trax, tray = splite(datax, datay, len(datax)/2)
        dt = DecisionTree()
        dt.max_depth = i
        # on fixe la  taille  de l'arbre a 5
        dt.fit(trax, tray)
        dt.predict(tesx)
        res[i-1] =  dt.score(datax, datay)
    #print len(res)
    x = np.arange(1, deptmax+1)
    #print len(x)
    plt.scatter(x,res)
    #plt.plot(x , res)
...
print dept_test(datax,datay,15)

def split_test(datax, datay , pas):
    res = np.zeros((len(datax)/pas)+1)
    cpt =0
    id = np.zeros((len(datax)/pas)+1)
    for i in range(1 , len(datax), pas):

        print cpt

        tesx, tesy, trax, tray = splite(datax, datay, i)
        dt = DecisionTree()
        dt.max_depth = 2
        # on fixe la
        dt.fit(trax, tray)
        dt.predict(tesx)
        id[cpt] = i
        res[cpt] = dt.score(datax, datay)
        cpt = cpt + 1
    plt.scatter(id , res)
    plt.xlabel('Profondeur')
    plt.ylabel('taille train')
    plt.show()
    return res , id
#print "test"
#print len(datax)/1000
print  split_test(datax, datay , 100)
#print len(datax)


def cross_val(nb_part, datax ):
    arr = np.arange(len(datax))
    np.random.shuffle(arr)
    arr =np.array_split(arr,nb_part)

    for i in range(nb_part-1):
        trax=np.array([datax[j] for j in           [:i,:],[datax[j] for j in  arr[1:,:])
        tray=
        tesx=
        tesy=
        dt = DecisionTree()
...
print datax[cross_val(10,datax)[1:]]
